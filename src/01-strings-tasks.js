...
function removeFirstOccurrences(str, value) {
  ...
  const valueEndIndex = valueIndex + value.length - 1;
  return str.slice(0, valueIndex) + str.slice(valueEndIndex + 1);
  // throw new Error('Not implemented');
}
...
function getRectangleString(width, height) {
  ...
  if (height > 2) {
    ...
    if (height === 3) {
      ...
      const innerLinesArr = new Array(height - 3).fill(innerLine);
      innerLines = `${innerLinesArr.join('\n')}\n${innerLine}\n`;
    }
  }
  return `┌${'─'.repeat(width - 2)}┐\n${innerLines}└${'─'.repeat(width - 2)}┘\n`;
  // throw new Error('Not implemented');
}
...
function isString(value) {
  try {
    return typeof value.slice() === 'string';
  } catch (error) {
    return false;
  }
  // throw new Error('Not implemented');
